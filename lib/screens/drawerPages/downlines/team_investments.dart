...
class TeamInvestmentPage extends StatefulWidget {
  const TeamInvestmentPage({super.key});

  @override
  State<TeamInvestmentPage> createState() => _TeamInvestmentPageState();
}

class _TeamInvestmentPageState extends State<TeamInvestmentPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      provider.teamInvestment(loading: true);
    });
  }
  ...
  Future<void> _loadMore() async {
    await provider.teamInvestment();
  }

  Future<void> _refresh() async {
    provider.incomePage = 0;
    await provider.teamInvestment( loading: false);
  }
  ...
  Widget build(BuildContext context) {
    return Consumer<TeamViewProvider>(builder: (context, provider, _) {
      return Scaffold(
        appBar: AppBar(
          title: titleLargeText('Team Investments', context, useGradient: true),
          actions: [],
        ),
        body: Container(
                  ...
                  children: [
                    if (!provider.loadingIncomeActivity)
                      _MyIncomeActivityHistoryList(
                          ...
                          onRetry: () => provider.teamInvestment(
                              loading: true)),
                    if (provider.loadingIncomeActivity)
                      Container(
...
class _MyIncomeActivityHistoryList extends StatelessWidget {
  ...
  final List<TeamInvestment> activities;
            ...
            : FixedTimeline.tileBuilder(
                ...
                builder: TimelineTileBuilder.connected(
                  ...
                  contentsBuilder: (_, index) {
                    TeamInvestment activity = activities[index];
                    return Padding(
                          ...
                          Builder(builder: (context) {
                            ...
                            return Column(
                              ...
                              children: [
                                ...
                                // ),
                                bodyMedText(
                                    '$currency${double.parse(activity.invoiceAmt ?? '0').toStringAsFixed(2)}',
                                    context),
                                height10(),
                                capText(
